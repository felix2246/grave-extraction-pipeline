...
Large data and model artifacts are no longer versioned in this repository so it can be pushed to GitHub. A fresh checkout does **not** contain everything needed to run the pipelines end-to-end.

Before running the code, you must provide or generate the required local artifacts yourself:

- input catalogue PDFs for the text and image pipelines
- `grave_extract.csv` generated by the text pipeline
- trained plate segmentation weights (`tafel_segmentation_model/.../model_final.pth` + `config.yaml`)
- trained sub-box ID detection weights (`tafel_subbox_id_extraction_model/.../model_final.pth` + `config.yaml`) if you want to use the A2-OOL matching strategy

Recommended execution order:

1. Add the input PDF(s) locally.
2. Run the text pipeline to generate `grave_extract.csv`.
3. Annotate/train the image models if the required weights do not exist yet.
4. Run the image pipeline with the generated CSV and locally trained model directories.
...
# 2) generate the grave extraction table
cd ../text_pipeline
uv run python scripts/run_pipeline.py \
  --pdf-path data/<catalogue>.pdf \
  --output-dir outputs/experiments/grave-extraction/<run-name> \
  --model mistral-large-instruct \
  --provider gwdg

# 3) train the image models when no weights are available yet
cd ../image_pipeline
uv run -m tafel_segmentation_model.train_net
uv run -m tafel_subbox_id_extraction_model.train_net

# 4) run the image pipeline with the generated artifacts
uv run python scripts/run_pipeline.py \
  --pdf-path grave_image_matching/data/<catalogue>.pdf \
  --graves-csv ../text_pipeline/outputs/experiments/grave-extraction/<run-name>/<generated-file>.csv \
  --output-dir grave_image_matching/output/<run-name> \
  --model mistral-large-instruct \
...
See the pipeline-specific READMEs for the exact data layout and training instructions:

- [`text_pipeline/README.md`](text_pipeline/README.md)
- [`image_pipeline/README.md`](image_pipeline/README.md)
...
Mount data, output, and model weight directories. The required CSV and model weights must exist locally first; they are not shipped with the repository.

```bash
docker build -t image-pipeline image_pipeline/

docker run --env-file image_pipeline/.env \
  -v $(pwd)/image_pipeline/grave_image_matching/data:/app/grave_image_matching/data \
  -v $(pwd)/image_pipeline/grave_image_matching/output:/app/grave_image_matching/output \
  -v $(pwd)/image_pipeline/tafel_segmentation_model/output:/app/tafel_segmentation_model/output:ro \
